...
def test_enviar_mensagem_success(stub_openai_client, agente):
    ...
    stub_openai_client.chat.completions.create.assert_called_once_with(
        model='gpt-4.1', messages=historico, temperature=0
    )
...
def test_transcrever_audio_sucesso(monkeypatch, stub_openai_client, agente, tmp_path):
...
def test_transcrever_audio_excecao_limpa_temp(monkeypatch, stub_openai_client, agente, tmp_path):
...
def test_baixar_imagem_como_data_uri(monkeypatch, agente, tmp_path):
    imagem_path = tmp_path / "foto.png"
...
def test_baixar_imagem_como_data_uri_falha(monkeypatch, agente):
    monkeypatch.setattr(agente, "_baixar_arquivo_temporario", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("download falhou")))

    with pytest.raises(DataProviderException):
        agente.baixar_imagem_como_data_uri("http://site/erro.png")


def test_baixar_arquivo_temporario_sem_extensao(monkeypatch, agente, tmp_path):
    original_mkstemp = provider_module.tempfile.mkstemp

    def fake_mkstemp(prefix="", suffix=""):
        return original_mkstemp(dir=tmp_path, prefix=prefix, suffix=suffix)
